...
function BackLetterContent(data){
    ...
    const sendQuerys = async() => {
        console.log('-------')
        try{
            const res = await axios.post(`${url}/post/detail/${data.data.tlcidxx}`,null, token)
    ...
    const go = async() => {
        if (data.data.lidx2 !== 0){
            history.push({
                pathname:"/main/GoLetter",
                state:{go_idx:data.data.lidx_2}
            })
        }else{
    ...
    useEffect(async()=>{
        console.log('________',data.data.lphoto)
        if (data.data.photo === undefined || data.data.photo === null){
    ...
    return(
        <div className="BackLetterContent">
            <div className="BackLetterContentInfo">
                <div style={{
                        backgroundImage:`url(${Img})`,
                        backgroundSize:'cover'
                    }}>
                </div>
                <div className="BackLetterContentInfoButton">
...
export default BackLetterContent;